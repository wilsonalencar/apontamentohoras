...
class contratacao extends app
{
	...
	private function checkExiste()
	{
		...
		$query = sprintf("SELECT codigo FROM contratacoes WHERE codigo = %d AND id <> %d", $this->codigo, $this->id);	
		
		if (!$result = $conn->query($query)) {
			$this->msg = "Ocorreu um erro durante a verificação do código da contratação";
			return false;	
		}
		
		if (!empty($result->fetch_array(MYSQLI_ASSOC))) {
			$this->msg = "Codigo da contratação já está sendo utilizado";
			return false;			
		}
		return true;
	}
	...
	private function check()
	{
		if (empty($this->codigo)) {
			$this->msg = "Insira o código da contratação.";
			return false;
		}

		if (empty($this->nome)) {
			$this->msg = "Insira o nome da contratação.";
			return false;
		}
	...
	{
		$conn = $this->getDB->mysqli_connection;
		$query = sprintf(" INSERT INTO contratacoes (codigo, nome, status, usuario)
		VALUES ('%s','%s','%s',%d)", 
	...
	{
		$conn = $this->getDB->mysqli_connection;
		$query = sprintf(" UPDATE contratacoes SET nome = '%s', status ='%s', usuario = %d, data_alteracao = NOW() WHERE id = %d", 
	...
	public function get($id)
	{
		...
		$conn = $this->getDB->mysqli_connection;
		$query = sprintf("SELECT id, codigo, nome, status, usuario FROM contratacoes WHERE id =  %d ", $id);
		if (!$result = $conn->query($query)) {
			$this->msg = "Ocorreu um erro no carregamento da contratação";	
			return false;	
		}
